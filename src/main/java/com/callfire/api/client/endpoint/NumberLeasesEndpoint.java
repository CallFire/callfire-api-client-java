 ...
 */
public class NumberLeasesEndpoint {
    ...
    private static final TypeReference<Page<NumberLease>> PAGE_OF_NUMBER_LEASE_TYPE = new TypeReference<Page<NumberLease>>() {
    };
    private static final TypeReference<Page<NumberConfig>> PAGE_OF_NUMBER_CONFIG_TYPE = new TypeReference<Page<NumberConfig>>() {
    };
    ...
    public NumberLeasesEndpoint(RestApiClient client) {
        this.client = client;
    }
    ...
    public Page<NumberLease> findNumberLeases(FindNumberLeasesRequest request) {
        return client.get(NUMBER_LEASES, PAGE_OF_NUMBER_LEASE_TYPE, request);
    }
     ...
     * Update number lease
     *
     * @param number      leased phone number to update
     * @param numberLease update payload
     * @throws CallfireApiException    in case API cannot be queried for some reason and server returned error
     * @throws CallfireClientException in case error has occurred in client
     */
    public void updateNumberLease(String number, NumberLease numberLease) {
        client.put(NUMBER_LEASES_ITEM.replaceFirst(PLACEHOLDER, number), VOID_TYPE, numberLease);
    }
    ...
    public Page<NumberConfig> findNumberLeaseConfigs(FindNumberLeaseConfigsRequest request) {
        return client.get(NUMBER_CONFIGS, PAGE_OF_NUMBER_CONFIG_TYPE, request);
    }
     ...
     * @throws CallfireClientException in case error has occurred in client
     */
    public void updateNumberLeaseConfig(String number, NumberConfig numberConfig) {
        client.put(NUMBER_CONFIGS_ITEM.replaceFirst(PLACEHOLDER, number), VOID_TYPE, numberConfig);
    }
